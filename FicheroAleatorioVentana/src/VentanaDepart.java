...
/**
 * Clase VentanaDepart: Esta clase dibuja la ventana
 * para la gestión de los datos del fichero AleatorioDep.dat
 * @author AARRMM 2013
 * @version 1-2013
 * */
public class VentanaDepart extends JFrame implements ActionListener, iVentanaDepart  {
	...
	/**
	 * Constructor. Dibuja la ventana 
	 * @param f, el frame donde la va a dibujar.
	 */
	 ...
	 * @param e Objeto del tipo ActionEvent, recibe el evento
	...
	public void actionPerformed(ActionEvent e) 	{   
		int dep, confirm;
		existeDepart = " DEPARTAMENTO EXISTEEE.";
		String deparError = " DEPARTAMENTO ERRÓNEO";
		if (e.getSource() == balta)
			alta(deparError);

		consultas(e);

		final String NOEXISTE_DEPART = " DEPARTAMENTO NO EXISTEEE.";
		if (e.getSource() == borra)
			borrar(deparError, NOEXISTE_DEPART);
		if (e.getSource() == modif)
			modificacion(deparError, NOEXISTE_DEPART);
	...
	@Override
	public void modificacion(String deparError, final String NOEXISTE_DEPART) {
		int dep;
		int confirm;
			...
			try {
				...
				if (dep >0)
					if (consultar(dep)) {
						...
						mensaje.setText(NOEXISTE_DEPART);	
	...
	@Override
	public void borrar(String deparError, final String NOEXISTE_DEPART) {
		int dep;
		int confirm;
	...
	@Override
	public void alta(String deparError) {
		int dep;
	...
	@Override
	public void consultas(ActionEvent e) {
		int dep;
			...
			try {
				...
				if (dep >0)
					if (consultar(dep)) {
						...
						mensaje.setText(" DEPARTAMENTO NO EXISTEEE.");	
						nombre.setText(" "); loc.setText(" ");
					}
				else mensaje.setText(" DEPARTAMENTO DEBE SER MAYOR QUE 0");	
	...
	/**
	 * Método verporconsola
	 * Este método visualiza los datos del fichero en la consola.
	 * @throws IOException
	 */
	public static void verporconsola() throws IOException {     
	...
	/**
	 * Método consultar
	 * Este método consulta los datos de un departamento que se recibe como parámetro.
	 * @param dep: es el número del departamento a consultar.
	...
	boolean consultar(int dep) throws IOException {		 
		...
		try { pos=44 * (dep-1);
		if (file.length()==0) { // si está vacío
			file.close();
			return false; 
		}
		file.seek(pos); 
	...
	void modificar(int dep) {
		...
		try {
			// Calculo del reg a leer
			pos=44 * (dep-1);
			new RandomAccessFile(fichero, "rw").seek(pos); 
			new RandomAccessFile(fichero, "rw").writeInt(dep);      
			nom=nombre.getText();
			loca=loc.getText();
			buffer = new StringBuffer(nom);      
			buffer.setLength(10); 
			new RandomAccessFile(fichero, "rw").writeChars(buffer.toString());
			buffer = new StringBuffer(loca);      
			buffer.setLength(10); 
			new RandomAccessFile(fichero, "rw").writeChars(buffer.toString());
			System.out.println("----REGISTRO MODIFICADOOO--------");
			new RandomAccessFile(fichero, "rw").close(); 
		}catch (IOException e1) {
